...
param p_jhMax {BUSES, GENS} >= 0;  #Pa_genMax
...
var p_jh {a in BUSES, j in GENS} >= 0;
...
#### dla kazdego a
subject to generation_limit22 {a in BUSES}:
Pa_gen[a] =  sum {j in GENS} p_jh[a,j];

#4 dla kazdego a
subject to generation_limit {a in BUSES, j in GENS}:
0 <= p_jh[a,j] <= p_jhMax[a,j];
